...
ADS1x15.prototype.start = function () {
    ...
    for (var i = 0; i < 4; i++) {
        ...
        switch (channelConfig.channelType) {
            case 'single':
                channelConfig.mux = ADS1015_REG_CONFIG_MUX_SINGLE_0 + (0x1000 * i);
                break;
            case 'diffTo1':
                channelConfig.mux = ADS1015_REG_CONFIG_MUX_DIFF_0_1;
                break;
            case 'diffTo3':
                channelConfig.mux = ADS1015_REG_CONFIG_MUX_DIFF_0_3 + (0x1000 * i);
                break;
            default:
                channelConfig.mux = 0;
                break;
...
ADS1x15.prototype.readAdc = function (index, callback) {
    ...
    if (that.ic == IC_ADS1015) {
        if (spsADS1015[channelConfig.samples]) {
            config |= spsADS1015[channelConfig.samples];
        } else {
            that.debug('Using default 250 SPS');
            config |= ADS1015_REG_CONFIG_DR_250SPS;
        }
    } else {
        if (spsADS1115[channelConfig.samples]) {
            config |= spsADS1115[channelConfig.samples];
        } else {
            that.debug('Using default 250 SPS');
            config |= ADS1115_REG_CONFIG_DR_250SPS;
        }
    }

    // Set PGA/voltage range
    if (pgaADS1x15[channelConfig.gain]) {
    ...
    that.writeWord(ADS1015_REG_POINTER_CONFIG, config);
    ...
    setTimeout(function() {
        var result = that.readWord(ADS1015_REG_POINTER_CONVERT);
        var value;
        if (that.ic == IC_ADS1015) {
